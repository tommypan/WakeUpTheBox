package gameEngine2D.loader
{
	...
	import gameEngine2D.cacher.SmartSourceCacher;
	import gameEngine2D.display.BitmapCacher;
	import gameEngine2D.display.BitmapFrameInfo;
	import gameEngine2D.display.BitmapMovie;
	import gameEngine2D.ge2d;
	import gameEngine2D.security.SecurityTool;
	 ...
	 * <p>changeSwfToBitmapMovie()	对很小的位图进行缩放，渲染效果不及原生movieClip。 
	...
	public class LoaderManager extends EventDispatcher
	{
		 ...
		 * @param isSo_ 是否存在用户磁盘上，下次打开游戏以变直接从用户电脑磁盘读取数据。当然这得用户同意我存储才行
		 * @param isRepeatSource_是否为重复资源，当希望得到动作一样的，资源一样的资源时，请将此参数设为true，下次申请资源时使用默认参数就行
		...
		public function getMovieClip(url_:String,$container:BitmapMovie,isCo_:Boolean=false,//回调参数再想一下
									 isSo_:Boolean=false,isRepeatSource_:Boolean=false,$callBack:Function=null):void
		{
			...
			nodeData.isCo = isCo_;
			nodeData.isSo = isSo_;
			nodeData.isRepeatSource_ = isRepeatSource_;
		...
		public function getBlurXml(url_:String,$callBack:Function=null,isCo_:Boolean=false,isSo_:Boolean=false):void
		{
			...
			{
				if($callBack is Function)		transFormXml(_cacher.getValue(url_) as ByteArray,$callBack);
			}else if(_sharedObj.data[url_]){
				if($callBack is Function) 	transFormXml(_sharedObj.data[url_] as ByteArray,$callBack);
			}else{
		 ...
		 * 
		 */		
		public function getModualSwf(url_:String,$callBack:Function=null):void
		{						
		...
		/**
		 *<p>将swf资源转换为bitmapMovie。慎用 </p>
		 * <p>一般一个大的swf里面可能封装一些小的swf，这时当加载进来时，需要通过反射机制取出swf所用。由于尺寸比较小的movieClip</p>
		 * <p>在转换为bitmap渲染时品质有一定损失，这样造成一定的不美观。所以，此方法慎用。建议直接通过反射机制将movieClip取出来操作</p>
		 * @param key 资源索引值
		...
		private function handleXml($node:LoaderQueueNode):void
		{
			...
			transFormXml(bytesArray,nodeData.callBack);
		...
		private function transFormXml(bytesArray :ByteArray,callBack:Function=null):void
		{
			if(callBack != null)
			{
