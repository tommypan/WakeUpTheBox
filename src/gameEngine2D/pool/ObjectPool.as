package  gameEngine2D.pool{
	...
	import gameEngine2D.pool.interfaces.IPool;
	...
	public class ObjectPool {
		...
		public function getObject(objParams:Object=null):IPool{
			//this._instanceParams = objParams;
			if (length == 0)
			{
				createInstance(_growupAmount);//没有了就先造默认个对象
				//return (this.getObject(this._instanceParams));
			};
			var obj:IPool = (_objectList.pop() as IPool);
			//weatherElement类实现了其接口，应用这些参数到mc
			//_local2.applyParams(this._instanceParams);
			return (obj);
		}
